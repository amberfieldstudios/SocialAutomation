...
export const PLATFORM_COPY: Record<WizardPlatformId, PlatformCopy> = {
  ...
  twitch: {
    ...
    registrationSteps: [
      ...
      'Client Type — choose "Confidential" (Twitch requires this so it can trust that only you can use the app).',
      'Click "Create", then open the application you just created.',
      'Copy the "Client ID" shown there and paste it below.',
      'Click "New Secret" and copy the app secret it shows you right away — Twitch only shows it once, so paste it below before leaving the page.',
    ],
  },
  reddit: {
    kind: 'guided',
